          ...
          # RF----
           fluidRow(
             box(width =12,
                 ...
                 numericInput("weight", "3. Birthweight (kg):", value=0, min=0, max=20),
                 prettyRadioButtons("prematurity", "4. Prematurity (i.e., gestational age < 37 weeks):", choices = list("Yes"=1, "No"=0), selected = 0, shape="round", inline= T),
                 prettyRadioButtons("BD", "5. Bronchopulmonary dysplasia:", choices = list("Yes"=1, "No"=0), selected = 0, shape="round", inline= T),
                 prettyRadioButtons("down_syndrome", "6. Down syndrome:", choices = list("Yes"=1, "No"=0),selected = 0, shape="round", inline= T),
                 prettyRadioButtons("chd", "7. Congenital heart disease:", choices = list("Yes"=1, "No"=0), selected = 0, shape="round", inline= T),
                 prettyRadioButtons("hiv", "8. HIV infection:", choices = list("Yes"=1, "No"=0), selected = 0, shape="round", inline= T),
                 prettyRadioButtons("siblings", "9. Number of siblings (defined as those living together with the child):", choices= list("0"= 0, "1"=1, "≥2"=2), selected = 0, shape="round", inline=T),
                 
                 prettyRadioButtons("smoking", "10. Maternal smoking during pregnancy:", choices = list("Yes"=1, "No"=0), selected = 0, shape="round", inline= T)
      ...
      ')),
          ...
          fluidRow( # 按照列布局
            column(
              br(),
              div(
                strong("RSV下呼吸道感染住院的风险"), # "you will find alternatives to learn all these technical aspects independently.",
                style="text-align:center;color:black;background-color:lavender;padding:15px;border-radius:10px;font-size: 16px;",
...
server <- function(input, output) {
  ...
  output$res_combiOR.hos <- renderUI({ # renderText
    ...
    if (isolate(input$siblings) == 2){ # 2对应的是label的值不是label值
      siblings2<-1}
    ...
    masterlist<- isolate(c(siblings2, siblings1, input$BD, weight, input$down_syndrome,input$prematurity, input$chd, input$hiv, input$sex, input$smoking)) # 注意顺序一定要与原来的危险因素保持一致
