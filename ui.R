# rm(list=ls())
library(shiny)
library(shinydashboard) # box函数
library(shinyWidgets)  # prettyRadioButtons函数
...
shinyUI( fluidPage(
    # 用于设置网页的标题和Logo
    tags$head(
      tags$link(rel = "shortcut icon", href = "logo.png", type = "image/png"), #Liyougroup
      tags$link(rel = "icon", href = "logo.png", type = "image/png"),
      tags$link(rel = "apple-touch-icon", href = "logo.png", type = "image/png")
    ),
    
  navbarPage(
    title = div(img(src = "logo.png", style = "height:30px;"),
                "RSVHeaRT"),
